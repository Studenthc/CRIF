...
class ConfigLastFMStar:
    def __init__(self):
        self.name = 'lastfm_star'
        self.use_gpu = True
        self.user_num = 1801
        self.item_num = 7432
        self.attribute_num = 8438
        self.parent_attribute_num = 8438
        self.nlayer = 2
        self.conv_name = 'gcn'
        self.n_heads = 1
        self.drop = 0.1
        ...
        self.top_taxo = 3
        self.feedback_aggregate = "gating"
        ...
        self.test_epoch_num = 30000

        self.input_dim = 4 + 15 + 8
        self.hidden_dim = 64
        self.output_dim = 2

        self.rec_model_path = root_path + "/recommendersystem/recmodel/lastfm_star"
    ...
    def _data_init(self):
        ...
        with open("./data/" + self.name + '/' + GRAPH_GENERATE_DATA + '/' + "/item_dict.json", "r") as f:
        ...
        if os.path.exists('utils/' + self.name + '/att_reverse_info.txt'):
            f = open('utils/' + self.name + '/att_info.txt', 'rb')
            att_info = pickle.load(f)
            f.close()

            f1 = open('utils/' + self.name + '/att_reverse_info.txt', 'rb')
        ...
        if need_att_info:
            att_info_file = open('utils/' + self.name + '/att_info.txt', 'wb')
            pickle.dump(att_info, att_info_file)
            att_info_file.close()
            att_reverse_info_file = open('utils/' + self.name + '/att_reverse_info.txt', 'wb')
        ...
        with open("./data/" + self.name + '/' + GRAPH_GENERATE_DATA + '/' + "/attribute_tree_dict.json", "r") as f:
