...
class ConfigYelp:
    def __init__(self):
        ...
        self.large_item_info = None
        self.large_att_info = None
        self._data_init()

    def _data_init(self):
        with open("./data/" + self.name + '/' + GRAPH_GENERATE_DATA + '/' + "/user_item.json", "r") as f:
        ...
        with open("./data/" + self.name + '/' + GRAPH_GENERATE_DATA + '/' + "/item_dict-original_tag.json", "r") as f:
        ...
        with open("./data/" + self.name + '/' + GRAPH_GENERATE_DATA + '/' + "/item_dict-merged_tag.json", "r") as f:
            large_item_info = json.load(f)
        self.large_item_info = {}
        self.large_att_info = {}
        for item in large_item_info:
            self.large_item_info[int(item)] = set(large_item_info[item])
            for att in large_item_info[item]:
                if len(self.large_att_info.get(int(att), set())) == 0:
                    self.large_att_info[int(att)] = set()
                    self.large_att_info[int(att)].add(int(item))
                else:
                    self.large_att_info[int(att)].add(int(item))

        attribute_tree_dict = {}
        with open("./data/" + self.name + '/' + GRAPH_GENERATE_DATA + '/' + 'first-layer_merged_tag_map.json', "r") as f:
            tag_map = json.load(f)
            with open("./data/" + self.name + '/' + GRAPH_GENERATE_DATA + '/' + "2-layer taxonomy.json", "r") as f1:
                taxonomy_dict = json.load(f1)
            for key, value in taxonomy_dict.items():
                head_id = tag_map[key]
                tail_ids = value
                attribute_tree_dict[head_id] = tail_ids
